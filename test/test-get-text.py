...
def _rebuild_scores(data):
    """Recompute ocr_scores.json leaderboard from raw data."""
    scores = {}
    for entry in data:
        key = entry["key"]
        if key not in scores:
            scores[key] = {"wins": 0, "fails": 0, "total_ms": 0.0}
        if entry["found"]:
            scores[key]["wins"] += 1
        else:
            scores[key]["fails"] += 1
        scores[key]["total_ms"] += entry["ms"]

    # Compute final leaderboard with avg speed
    leaderboard = {}
...
def _record_run(setting, elapsed, found):
...
def _sort_settings_by_score():
    """Return SETTINGS ordered by score: more wins + faster = higher priority."""
    scores = _load_scores()

    def _score(setting):
        entry = scores.get(_setting_key(setting))
        if entry is None:
            return (0, 0.0, 999.0)  # no history → lowest priority
...
def get_text(
    ...
    ordered = _sort_settings_by_score()
    all_results = []
    ...
    for batch_start in range(0, len(ordered), batch_size):
        ...
        with ThreadPoolExecutor(max_workers=len(batch)) as pool:
            ...
            for future in as_completed(futures):
                ...
                _record_run(setting, elapsed, bool(found))
                if debug:
